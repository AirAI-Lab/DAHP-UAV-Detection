...
DAHP-Slice 评估工具
====================
支持三种推理模式, 全部输出 COCO 标准指标 (AP50-95/AP50/AP75/APs/APm/APl/逐类AP):
  full   : 全图推理 (--imgsz)
  slice  : 固定网格切片推理 (SAHI 风格, --slice/--overlap/--slice-imgsz)
  hybrid : 全图 + 切片预测合并 (大目标用全图兜底)
...
def wbf_merge(boxes, scores, classes, iou_thr):
    """Weighted Box Fusion, 返回融合后的 (boxes, scores, classes)"""
...
def predict_sliced(model, img, slice_, overlap, slice_imgsz, conf, max_det):
    """img: BGR ndarray. 返回全局坐标下的 (boxes, scores, classes)"""
    H, W = img.shape[:2]
    crops, metas = [], []
...
def coco_eval(gt_coco, pred_json, per_class=True, maxdets=300):
    ...
    e.accumulate()
    e.summarize()
    stats = _stats_from_eval(e)  # 绕开 summarize() 对默认 maxDets=100 的硬编码
...
def _stats_from_eval(e):
    """直接从 COCOeval 的 precision/recall 数组计算指标.

    pycocotools summarize() 的全局 AP 硬编码 maxDets=100 查找,
    当 maxDets=[1,10,N] 且 N!=100 时返回 -1, 故手动计算.
...
def main():
    ...
    p.add_argument("--models", type=str, default=None,
                   help="ensemble 模式: 逗号分隔的多个 best.pt (覆盖 --model)")
    p.add_argument("--slice-model", type=str, default=None,
                   help="hybrid 模式下切片推理用的第二模型 (默认与 --model 相同)")
    ...
    p.add_argument("--imgsz", type=int, default=1280, help="全图推理尺寸")
    p.add_argument("--slice", type=int, default=640, help="切片尺寸")
    p.add_argument("--overlap", type=float, default=0.2)
    p.add_argument("--slice-imgsz", type=int, default=None, help="切片推理尺寸, 默认=切片尺寸")
    ...
    p.add_argument("--tta", action="store_true", help="full 模式启用 TTA (augment)")
    ...
    if args.mode == "ensemble":
        assert args.models, "ensemble 模式需要 --models path1,path2,..."
    ...
    for img_id, img_path in enumerate(tqdm(img_files, desc="infer"), start=1):
        ...
        # 融合 (full 模式单源预测已被 predict 内部 NMS@0.7 处理, 不再重复)
