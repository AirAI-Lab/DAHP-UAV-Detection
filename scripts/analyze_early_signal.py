#!/usr/bin/env python3
"""Retrospective early-signal analysis of two-arm regime separation.

The script measures whether the early (epoch-10..40) gap between a rebalanced
arm and its matched base is correlated with the eventual gain.  The analysis is
retrospective: it does not simulate a prospective policy decision.
"""
import csv, json
...
PAIRS = {  # res -> (base run, treated run, label)
    640: ("base_640", "union_640", "union"),
    960: ("base_960", "union_960", "union"),
    1280: ("base_1280", "tail_1280", "tail"),
    1600: ("base_1600", "union_1600", "union"),
    1920: ("base_1920", "union_1920", "union"),
}
PROBE_EPOCHS = [10, 15, 20, 25, 30, 40, 50]

def load_map(run):
    f = BASE / "runs" / run / "results.csv"
...
json.dump(
    {
        "analysis": "retrospective_early_signal",
        "pairs": out,
        "correlation": corr,
    },
    open(BASE / "results/early_signal_analysis.json", "w"),
    indent=1,
)
...
ax.set_title("(b) Early gap versus eventual gain", fontsize=10)
ax.legend(fontsize=7)
plt.tight_layout()
plt.savefig(BASE / "paper/figures/fig9_early_signal.pdf")
plt.savefig(BASE / "paper/figures/fig9_early_signal.png", dpi=300)
print("saved fig9_early_signal")
