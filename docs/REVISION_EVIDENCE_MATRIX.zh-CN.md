...
| RQ3 | rebalancing 效果是否依赖 regime？ | 640/960/1600/1920；online probe r=0.983 | 同 detector、epoch、评估协议 | **Mostly supported** | exact 1280 controls 在跑 |
| RQ4 | 增益来自曝光还是 targeted sampling？ | 1600 md100: R 36.55 / random 36.91 / targeted 37.09；1920 exposure-matched control | random-volume 与 120-epoch base control | **Partially supported** | targeted n=3，待 n=5 |
...
| exposure/capacity/learnability 混淆 | 640 learnability、1920 exposure matching、capacity ladder | 强 | 完成 exact 1280 |
...
2. **Volume-driven rescue**：960 px 下 union +3.15 AP，volume 成分主导。
3. **Saturated behavior**：当前 1280 proxy 接近 0；exact controls pending。
...
| 实验支持 | 大部分 | 五个分辨率中四个强；exact 1280 pending |
...
- [ ] targeted union n=5
- [ ] exact 1280 union/random controls
- [ ] RT-DETR union cross-family result
- [ ] n=5 后最终 attribution table
- [ ] exact 1280 后最终 regime matrix / Fig. 8
