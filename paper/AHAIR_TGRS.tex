...
\begin{table}[!t]
\centering
\caption{Union-sampling effect across the resolution--regime matrix on the same YOLOv8m-P2 detector family (legacy native protocol, matched 60/100-epoch budgets per series). The effect tracks data-exposure headroom, not resolution alone. The exact same-evaluator 1280 triplet is reported separately in Table~\ref{tab:attribution} to avoid protocol mixing.}
\label{tab:lawmatrix}
...
base@120 (exposure-matched) & --- & --- & --- & --- & 39.01$^{d}$ \\
\midrule
+$\Delta$ (same epoch) & $+0.21$ & $+3.15$ & $+0.05^{c}$ & $+0.78$ & $+3.83$ \\
+$\Delta$ (exposure-matched) & --- & --- & --- & --- & $+0.49$ \\
+regime & unlearn. & under-fit & volume-sat. & sweet & expos.-lim. \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^{c}$legacy frequency-sampling proxy at 1280 (36.25 vs.\ 36.20); exact 1280 base/random/union is $34.63/34.72/35.12$}\\
\multicolumn{6}{l}{\footnotesize md100. $^{d}$md100: base@120 $=37.51$, union@60 $=37.93$ ($+0.42$); \aps{} $29.54\to30.46$ ($+0.92$).}
\end{tabular}}
\end{table}
