...
\begin{table}[!t]
\centering
\caption{Factor attribution and component ablation. The $V_L$/DAHP-L block and exposure decomposition use COCO md100; the DAHP-component block uses the native/md300 training-selection protocol. All schedules are 100 epochs unless stated.}
...
\midrule
+$\Delta$ (same epoch) & $+0.21$ & $+3.15$ & $\approx0$ & $+0.78$ & $+3.83$ \\
+$\Delta$ (exposure-matched) & --- & --- & --- & --- & $+0.49$ \\
+regime & unlearn. & under-fit & saturated & sweet & expos.-lim. \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^{c}$frequency-sampling proxy at 1280 (36.25 vs.\ 36.20); union at 640/960/1600/1920. $^{d}$md100: base@120 $=37.51$, union@60 $=37.93$ ($+0.42$); \aps{} $29.54\to30.46$ ($+0.92$).}
\end{tabular}}
\end{table}
...
\caption{The five-point resolution--sampling matrix. (a) base vs.\ union sampling across resolutions; (b) the same-epoch effect tracks \emph{data-exposure headroom}, not resolution. The 1920 bar is an apparent same-epoch gain; the exposure-matched residual is $+0.49$ native / $+0.42$ md100 \ap.}
\label{fig:lawmatrix}
